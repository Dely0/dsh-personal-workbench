...
import { extractTerms, RECALL_DEFAULTS, recallKnowledge } from './shared/knowledgeRecall.js'
...
export function searchKnowledgeTool(manager: KnowledgeRecallManager) {
  return defineTool({
    ...
    description:
      ...
      + '排序规则：本任务/本任务树优先，其次全库；相关度 0~1（标题命中 0.55 / 标签 0.45 / 正文 0.30 + 补充项）。'
      + '结果里的 [id] 可用于 report_usage 回报引用。',
    parameters: {
      query: { type: 'string', required: true, description: '检索关键词（现象词/报错原文片段/模块名/函数名都行；中文逐字匹配）' },
      task_id: { type: 'string', description: '限定"本任务"的范围（可选；不传则按当前会话关联的任务推断）' },
      limit: { type: 'number', description: '返回条数，默认 8，最大 30' },
      min_score: { type: 'number', description: `最低相关度，默认 ${RECALL_DEFAULTS.minScore}（低于它的不返回）` },
    },
    output: {
      schema: { type: 'string' },
    ...
    async execute(args: Record<string, unknown>, exec: ToolExec) {
      ...
      const outcome = recallKnowledge({
        ...
        minScore: typeof args.min_score === 'number' ? args.min_score : RECALL_DEFAULTS.minScore,
        maxEntries: 30,
      })
      ...
      manager.logSearch({
        ...
        query,
        terms,
        hits: limited.map((hit) => ({ id: hit.id, title: hit.title, score: hit.score })),
        matched: outcome.matched,
        droppedByScore: outcome.droppedByScore,
      })
      if (limited.length === 0) {
        return `检索「${query}」零命中（关键词：${terms.join('、') || '无'}；候选池 ${outcome.matched} 条，无一条过阈值）。`
      ...
      for (const hit of limited) {
        lines.push(`- [${hit.id}] ${hit.title}（${hit.kindCode} · 相关度 ${hit.score.toFixed(2)}）`)
    ...
    async execute(args: Record<string, unknown>, exec: ToolExec) {
      ...
      if (action === 'status') {
        ...
        const hits = state.lastHits.length === 0
          ? '（本会话还没有命中记录）'
          : state.lastHits.map((hit) => `\n- [${hit.id}] ${hit.title}（${hit.score.toFixed(2)} · ${hit.reason}）`).join('')
