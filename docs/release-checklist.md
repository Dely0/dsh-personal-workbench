...
> **完整发布门禁另有一份可复用的用户级 skill**：`dsh-release`（本机 `~/.dsh/skills/dsh-release/`）。
> 它把本节 + 网络现实 + 凭据链 + GitHub Release 通道 + 编码坑串成一条命令序列，
> 并带一个脚本 `scripts/new-github-release.ps1`（Title 硬校验 / UTF-8 正文 / 凭据回退 / `releases/latest` 复核）。
